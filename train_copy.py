        ...
        for i, datas in enumerate( \
            ...
            if step_cnt % save_interval == 0:
                for i in range(density_map.shape[0]):
                    density_gen.save_image(raw_patch[i], opt.expr_dir + './sup/',
                                           'img_step%d_%d_0data.jpg' % (step_cnt, i))
                    density_gen.save_density_map(gt_data[i], opt.expr_dir + "./sup/",
                                                 'img_step%d_%d_1previous.jpg' % (step_cnt, i))
                    density_gen.save_density_map(density_map[i], opt.expr_dir + "./sup/",
                                                 'img_step%d_%d_2now.jpg' % (step_cnt, i))
                for i in range(density_map_after.shape[0]):
                    density_gen.save_density_map(density_map_after[i], opt.expr_dir + "./sup/",
